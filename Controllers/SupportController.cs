...
namespace PawsPort.Controllers
{
    ...
    public class SupportController : ApiControllerBase
    {
        ...
        [HttpPost("Qa")]
        [Tags("客服管理")]
        public async Task<IActionResult> CreateQa([FromBody] QaCreateDTO dto)
        {
            if (!ModelState.IsValid)
            {
            ...
            var isSuccess = await _qaService.CreateQaAsync(dto);

            if (!isSuccess)
            {
                return Failure("CREATE_FAILED", "新增問答紀錄失敗，請稍後再試", 500);
            }

            return Success<object>(null, "QA提問新增成功", 200);
        }
