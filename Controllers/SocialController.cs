        ...
        public IActionResult SocialList(SocialListViewModel vm)
        {
            using (PetDbContext db = new PetDbContext())
            {
                ...
                //將查到的使用者轉換成SocialItemViewModel p
                {
                    ...
                    TotalViewCount = db.Articles
                    .Where(a => a.UserId == p.UserId).Sum(a => (int?)a.ViewCount) ?? 0, //發文總閱覽數
                    ...
                    //人氣分數算法 (粉絲*10+書籤*5+留言*2+總閱覽數/100)
                 ...
                 ((db.Articles
                    .Where(a => a.UserId == p.UserId).Sum(a => (int?)a.ViewCount) ?? 0)/100)
                });
                ...
                switch (vm.SortBy) //根據vm.SortBy的值來決定排序方式
                {
                    ...
                    case "Default": //預設排序，按照使用者ID由小到大排序
                        if (vm.SortOrder == "desc")
                        {
                            ItemQuery = ItemQuery.OrderByDescending(i => i.UserName);
                        }
                        else
                        {
                            ItemQuery = ItemQuery.OrderBy(i => i.UserName);
                        }
        ...
        public IActionResult Details(int id)
        {
            using (PetDbContext db = new PetDbContext())
            {
                ...
                var vm = new SocialDetailsViewModel //將使用者資料轉換成SocialDetailsViewModel
                {
                    ...
                    RecentArticles = db.Articles.Where(a => a.UserId == id) //列出最近的貼文
                    .OrderByDescending(a => a.CreateAt).Take(5) //由新到舊取出5篇
                    .Select(a => new ArticleSummary //show出貼文的資料
                    {
